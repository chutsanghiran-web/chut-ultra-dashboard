...
st.caption("Version 3.2: Heart Rate Lab, Pattana readiness, Running Lab, KC dynamics, DD/MM/YYYY dates, and detailed race readiness.")
st.sidebar.header("Navigation")
page = st.sidebar.radio(
    "Go to",
    ["Home", "Training Plan", "Activities", "KC Tracker", "Running Lab", "Heart Rate Lab", "Recovery", "Race Readiness", "AI Coach"]
)
...
# Heart rate settings based on your Garmin / personal data
USER_RESTING_HR = 50
USER_MAX_HR = 208
USER_HR_RESERVE = USER_MAX_HR - USER_RESTING_HR

def hr_zone_from_avg(avg_hr):
    if avg_hr is None or pd.isna(avg_hr):
        return "Unknown"
    pct_max = avg_hr / USER_MAX_HR * 100

    if pct_max < 60:
        return "Z1 / Recovery"
    elif pct_max < 70:
        return "Z2 / Easy Aerobic"
    elif pct_max < 80:
        return "Z3 / Moderate"
    elif pct_max < 90:
        return "Z4 / Hard"
...
def hr_lab_comment(row):
    ...
    if group == "KC/Trail":
        if pct_max >= 80:
            return "High HR is expected for KC/trail climbing. Compare this mainly against future KC sessions, not road runs."
        return "Controlled KC/trail effort. Good sign if climbing pace improves without HR rising."

    if group == "Running":
        if pct_max >= 80:
            return "High for a road run. Possible causes: heat/humidity, fatigue, recent illness, low aerobic base, or pushing too hard."
        elif pct_max >= 70:
            return "Moderate road-run effort. Useful aerobic work, but not truly easy."
        else:
            return "Easy aerobic effort. This is the kind of HR range that builds base."

    if group == "Cycling":
        return "Cycling HR is useful for aerobic load but should not be compared directly to running HR."

    if group == "Swimming":
        return "Swimming HR can be less reliable on wrist devices; use effort feel too."

    return "Heart-rate effort logged."
...
def hr_zone_table():
    ...
    zone_defs = [
        ...
        ("Z4 / Hard", 0.80, 0.90),
        ("Z5 / Very Hard", 0.90, 1.00),
    ]
    for name, low, high in zone_defs:
        low_hr = round(USER_RESTING_HR + USER_HR_RESERVE * low)
        high_hr = round(USER_RESTING_HR + USER_HR_RESERVE * high)
        zones.append({
            "Zone": name,
            "HR range": f"{low_hr}-{high_hr} bpm",
            "% HR Reserve": f"{int(low*100)}-{int(high*100)}%",
        })
...
if not activities.empty:
    ...
    if "avg_hr" in activities.columns:
        activities["hr_pct_max"] = (activities["avg_hr"] / USER_MAX_HR * 100).round(1)
        activities["hr_pct_reserve"] = ((activities["avg_hr"] - USER_RESTING_HR) / USER_HR_RESERVE * 100).round(1)
        activities["hr_zone"] = activities["avg_hr"].apply(hr_zone_from_avg)
        activities["hr_lab_comment"] = activities.apply(hr_lab_comment, axis=1)
else:
...
if page == "Home":
...
elif page == "Heart Rate Lab":
    st.header("❤️ Heart Rate Lab")
    st.caption("Compares your heart rate across different types of training. Uses your resting HR = 50 and max HR = 208.")

    c1, c2, c3 = st.columns(3)
    c1.metric("Resting HR", f"{USER_RESTING_HR} bpm")
    c2.metric("Max HR", f"{USER_MAX_HR} bpm")
    c3.metric("HR Reserve", f"{USER_HR_RESERVE} bpm")

    st.subheader("Your Heart Rate Zones")
    st.dataframe(hr_zone_table(), use_container_width=True)

    if activities.empty or "avg_hr" not in activities.columns:
        st.info("No activity heart-rate data loaded yet.")
    else:
        hr_df = activities.dropna(subset=["avg_hr"]).copy()

        st.subheader("Average HR by Activity")
        fig = px.scatter(
            hr_df,
            x="date",
            y="avg_hr",
            ...
            hover_data={
                ...
                "duration_min": ":.1f",
                "avg_hr": True,
                "max_hr": True if "max_hr" in hr_df.columns else False,
                "hr_pct_max": True,
                "hr_pct_reserve": True,
                "hr_zone": True,
                "avg_pace_min_km": True if "avg_pace_min_km" in hr_df.columns else False,
                "ascent_m": True if "ascent_m" in hr_df.columns else False,
                "hr_lab_comment": True,
                "activity_group": False,
            },
            title="Average Heart Rate by Session"
        )
        fig.update_layout(
        ...
        st.subheader("Effort Comparison by Activity Type")
        summary = hr_df.groupby("activity_group", as_index=False).agg(
            sessions=("date", "count"),
            avg_hr=("avg_hr", "mean"),
            avg_hr_pct_max=("hr_pct_max", "mean"),
            avg_hr_pct_reserve=("hr_pct_reserve", "mean"),
            avg_distance_km=("distance_km", "mean"),
            total_hours=("duration_min", lambda x: x.sum() / 60),
        )
        summary["avg_hr"] = summary["avg_hr"].round(0)
        summary["avg_hr_pct_max"] = summary["avg_hr_pct_max"].round(1)
        summary["avg_hr_pct_reserve"] = summary["avg_hr_pct_reserve"].round(1)
        summary["avg_distance_km"] = summary["avg_distance_km"].round(2)
        summary["total_hours"] = summary["total_hours"].round(2)
        st.dataframe(summary, use_container_width=True)

        st.subheader("Road Run Comparison")
        road = hr_df[hr_df["activity_group"] == "Running"].copy()
        if road.empty:
            st.info("No road runs classified yet. Tell me when a run is road vs KC and we will keep this clean.")
        else:
            road_cols = [
                "date", "distance_km", "duration_min", "avg_hr", "max_hr",
                "hr_pct_max", "hr_pct_reserve", "hr_zone",
                "avg_pace_min_km", "hr_lab_comment"
            ]
            road_cols = [c for c in road_cols if c in road.columns]
            road_display = road[road_cols].sort_values("date", ascending=False).copy()
            road_display["date"] = fmt_date_series(road_display["date"])
            st.dataframe(road_display, use_container_width=True)

        st.subheader("AI Heart Rate Assessment")
        st.write("- Your max HR of 208 means a run averaging 160 bpm is about 77% of max HR. That is moderate-to-hard, not automatically dangerous.")
        st.write("- Your resting HR of 50 is a good sign and does not suggest you are simply unfit.")
        st.write("- In Thailand heat and humidity, road-run HR can easily run 5-15 bpm higher than cooler conditions.")
        st.write("- The key trend to watch is whether pace improves at the same HR over time, especially on road runs.")
        st.write("- KC/trail HR should be compared against other KC/trail sessions, not flat road runs.")
    ...
    st.subheader("Heart Rate Coach Snapshot")
    st.write("- Resting HR: **50 bpm**")
    st.write("- Max HR: **208 bpm**")
    st.write("- Main dashboard goal: compare HR against similar efforts, especially road vs road and KC vs KC.")
    st.write("- High HR does not automatically mean poor fitness; heat, humidity, hills, fatigue, and recent illness all matter.")
